<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="产品编号" prop="chanpinbianhao">
          <el-select :disabled="ro.chanpinbianhao" @change="chanpinbianhaoChange" v-model="ruleForm.chanpinbianhao" placeholder="请选择产品编号">
            <el-option
                v-for="(item,index) in chanpinbianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="工艺名称" prop="gongyimingcheng">
          <el-input v-model="ruleForm.gongyimingcheng" 
              placeholder="工艺名称" clearable  :readonly="ro.gongyimingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="工艺名称" prop="gongyimingcheng">
              <el-input v-model="ruleForm.gongyimingcheng" 
                placeholder="工艺名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="加工原料" prop="jiagongyuanliao">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="加工原料"
                  v-model="ruleForm.jiagongyuanliao" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jiagongyuanliao" label="加工原料" prop="jiagongyuanliao">
                    <span>{{ruleForm.jiagongyuanliao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="工艺流程" prop="gongyiliucheng">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.gongyiliucheng" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.gongyiliucheng" label="工艺流程" prop="gongyiliucheng">
                    <span v-html="ruleForm.gongyiliucheng"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="工作原理" prop="gongzuoyuanli">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.gongzuoyuanli" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.gongzuoyuanli" label="工作原理" prop="gongzuoyuanli">
                    <span v-html="ruleForm.gongzuoyuanli"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	gongyimingcheng : false,
	jiagongyuanliao : false,
	gongyiliucheng : false,
	gongzuoyuanli : false,
      },
      ruleForm: {
        chanpinbianhao: '',
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chanpinleixing = true;
          ...
          if(o=='gongyimingcheng'){
            this.ruleForm.gongyimingcheng = obj[o];
	    this.ro.gongyimingcheng = true;
            continue;
          }
          if(o=='jiagongyuanliao'){
            this.ruleForm.jiagongyuanliao = obj[o];
	    this.ro.jiagongyuanliao = true;
            continue;
          }
          if(o=='gongyiliucheng'){
            this.ruleForm.gongyiliucheng = obj[o];
	    this.ro.gongyiliucheng = true;
            continue;
          }
          if(o=='gongzuoyuanli'){
            this.ruleForm.gongzuoyuanli = obj[o];
	    this.ro.gongzuoyuanli = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/chanpinxinxi/chanpinbianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.chanpinbianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    chanpinbianhaoChange () {
      this.$http({
        url: `follow/chanpinxinxi/chanpinbianhao?columnValue=`+ this.ruleForm.chanpinbianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.chanpinmingcheng){
                this.ruleForm.chanpinmingcheng = data.data.chanpinmingcheng
              }
              if(data.data.chanpinleixing){
                this.ruleForm.chanpinleixing = data.data.chanpinleixing
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.gongyiliucheng = this.ruleForm.gongyiliucheng.replace(reg,'../../../springboot55nqb/upload');
	this.ruleForm.gongzuoyuanli = this.ruleForm.gongzuoyuanli.replace(reg,'../../../springboot55nqb/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gongyixianlu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gongyixianluCrossAddOrUpdateFlag = false;
						 this.parent.search();
